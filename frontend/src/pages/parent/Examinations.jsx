...
import { Filter, List, Loader2, CalendarDays, Clock, AlertTriangle, Users, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function Examinations() {
  ...
  const [filters, setFilters] = useState({ exam_type: '' });
  ...
  const fetchExams = async (page = 1, activeFilters = filters, childId = selectedChild) => {
    const childObj = children.find(c => String(c.id) === childId);
    const classId = childObj?.current_enrollment?.class_id;
    ...
    setError(null);
    try {
      const params = { class_for: classId, page, page_size: PAGE_SIZE };
  ...
  // Fetch exams + types when selected child changes
  useEffect(() => {
    ...
    const init = async () => {
      ...
      setCurrentPage(1);
      fetchExams(1, filters, selectedChild);
    };
    init();
  }, [selectedChild]);
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] p-2 md:p-4">
      ...
      <header className="mb-4">
        ...
        <p className="mt-1 text-slate-500 font-medium">View your child's examination schedule</p>
      </header>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        <div className="flex items-center gap-2 border-b border-slate-100 p-5 font-bold text-slate-800 bg-slate-50/50">
          ...
          <span>
            {selectedChildObj
              ? `${selectedChildObj.user?.first_name || 'Child'}'s Examinations`
              : 'All Examinations'}
          </span>
